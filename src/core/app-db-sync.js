/**
 * In-process artifact → SQLite app-DB sync.
 *
 * This is the cron-safe sibling of web/lib/sync.ts. It performs the same
 * idempotent upserts into ~/.tradingview-mcp/app.db but runs inside the
 * scheduler Node process directly — no subprocess, no npm, no PATH
 * dependency. macOS cron's minimal environment cannot find `npm`, which
 * is why the previous `execSync('npm run db:sync')` implementation failed
 * with "/bin/sh: npm: command not found".
 *
 * Schema source of truth: web/db/schema.sql (read at runtime so the two
 * sync implementations can never drift).
 *
 * Behavior:
 *   - Initializes the DB from web/db/schema.sql if missing.
 *   - Upserts premarket / postclose / analytics / breakdowns / models /
 *     shadow predictions / system status via ON CONFLICT.
 *   - Never throws on missing files / malformed JSON — returns counts
 *     and a status instead.
 *   - Appends a row to sync_runs for every invocation.
 */
...
const SCHEMA_PATH = join(WEB_DIR, 'db', 'schema.sql');
...
const GRADES_LOG       = join(HOME_DIR, 'performance', 'daily_grades.jsonl');
const DATASETS_SUMMARY = join(HOME_DIR, 'datasets', 'dataset_summary.json');
const DB_PATH          = join(HOME_DIR, 'app.db');
...
const MODEL_TASKS = [
  'bias_direction','day_type','range_in_tolerance','actual_range_points','good_grade',
];
...
function getDb() {
  ...
  if (existsSync(SCHEMA_PATH)) {
    try { _db.exec(readFileSync(SCHEMA_PATH, 'utf8')); } catch { /* already exists */ }
  }
  return _db;
}
...
function syncReports() {
  const db = getDb();
  const dates = listDirSafe(REPORTS_DIR).filter(d => /^\d{4}-\d{2}-\d{2}$/.test(d)).sort();
  ...
  return { premarket: pmN, postclose: pcN };
}
...
function syncModels() {
  ...
  for (const task of MODEL_TASKS) {
    ...
    const status  = readJsonSafe(join(taskDir, 'training_status.json'));
    const champ   = readJsonSafe(join(taskDir, 'champion.json'));
    if (!status && !champ) continue;
    stmt.run({
      task,
      status:             status?.status ?? 'not_trained',
      champion_name:      champ?.candidate_name ?? champ?.baseline_kind ?? null,
      champion_family:    champ?.family ?? champ?.baseline_kind ?? null,
      is_baseline:        asIntBool(champ?.is_baseline),
      champion_metric:    champ?.champion_metric ?? null,
      validation_metric:  champ?.validation_metric_value ?? null,
      test_metrics_json:  champ?.test_metrics ? JSON.stringify(champ.test_metrics) : null,
...
// ─── syncShadowPredictions ───────────────────────────────────────────────────

function syncShadowPredictions() {
  const db = getDb();
  const stmt = db.prepare(`
  ...
  const ingest = (snapshot, sourcePath) => {
    if (!snapshot || typeof snapshot !== 'object') return 0;
    let n = 0;
    const tradingDate = snapshot.trading_date ?? null;
    const preds = snapshot.predictions ?? {};
    for (const [task, p] of Object.entries(preds)) {
  ...
  for (const snap of readJsonlSafe(histPath)) n += ingest(snap, histPath);
  return n;
}
...
function syncSystemStatus() {
  ...
  const analyticsPath = join(ANALYTICS_DIR, 'summary.json');
  const analytics = readJsonSafe(analyticsPath);
  entries.push({
    key: 'last_analytics',
    value: analytics?.last_updated ?? null,
    last_updated: statMtime(analyticsPath),
    details_json: analytics ? JSON.stringify({ total_days_graded: analytics.total_days_graded }) : null,
  });

  const ds = readJsonSafe(DATASETS_SUMMARY);
  entries.push({
    key: 'last_dataset',
    value: ds?.last_updated ?? null,
    last_updated: statMtime(DATASETS_SUMMARY),
    details_json: ds ? JSON.stringify({ counts: ds.counts, date_range: ds.date_range }) : null,
  });

  const modelSummaryPath = join(MODELS_DIR, 'training_summary.json');
  const modelSummary = readJsonSafe(modelSummaryPath);
  entries.push({
  ...
  const shadow = readJsonSafe(shadowPath);
  entries.push({
  ...
  entries.push({ key: 'last_sync', value: nowISO(), last_updated: nowISO(), details_json: null });
...
function recordSyncRun(started_at, finished_at, status, counts, error) {
  const db = getDb();
  db.prepare(`
...
/**
 * Sync every pipeline artifact into the app DB, in-process.
 * Never throws. Returns a structured result; callers log and continue.
 */
export function syncAllArtifacts() {
  const started_at = nowISO();
  const counts = { premarket: 0, postclose: 0, snapshots: 0, breakdowns: 0, models: 0, shadow: 0, system: 0 };
  try {
    // Check web layer is present (schema file must exist)
    if (!existsSync(SCHEMA_PATH)) {
      const finished_at = nowISO();
      return {
        success: false,
        skipped: true,
        reason:  'web/db/schema.sql not found — app layer not installed',
        counts,
        started_at,
        finished_at,
      };
    ...
    return { success: true, counts, db_path: DB_PATH, started_at, finished_at };
  } catch (err) {
    const finished_at = nowISO();
    const msg = err?.message ?? String(err);
    try { recordSyncRun(started_at, finished_at, 'error', counts, msg); } catch { /* db may be unavailable */ }
    return { success: false, counts, error: msg, started_at, finished_at };
  }
}

// Convenience for callers that want to close the DB (useful when the parent
// process is about to exit and wants to flush WAL).
